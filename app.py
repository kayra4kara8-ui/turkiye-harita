...
treemap_df["Strateji_Kısa"] = (
    treemap_df["Yatırım Stratejisi"]
    .str.replace("🚀 ", "")
    .str.replace("⚡ ", "")
    .str.replace("🛡️ ", "")
    .str.replace("💎 ", "")
...
with col_bcg1:
    ...
    def assign_bcg(row):
        if row["Toplam Kutu"] >= pazar_median and row["Pazar Payı %"] >= pay_median:
            return "⭐ Stars"
        elif row["Toplam Kutu"] >= pazar_median:
            return "❓ Question Marks"
        elif row["Pazar Payı %"] >= pay_median:
            return "💰 Cash Cows"
        else:
            return "🐕 Dogs"

    scatter_df["BCG Kategori"] = scatter_df.apply(assign_bcg, axis=1)

    min_val, max_val = scatter_df["PF Kutu"].min(), scatter_df["PF Kutu"].max()
    scatter_df["Nokta Boyutu"] = (
        35 if max_val == min_val
        else 20 + (scatter_df["PF Kutu"] - min_val) / (max_val - min_val) * 40
    )
    ...
    fig_bcg.add_hline(y=pay_median, line_dash="dash")
    fig_bcg.add_vline(x=pazar_median, line_dash="dash")
...
with col_bcg2:
    st.success("⭐ **Stars** → Büyümeye devam")
    st.info("❓ **Question Marks** → Yatırım fırsatı")
    st.warning("💰 **Cash Cows** → Stabil gelir")
    st.error("🐕 **Dogs** → Düşük öncelik")
