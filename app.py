...
REGION_COLORS = {
    ...
    "BATI ANADOLU": "#14B8A6",         # Turkuaz
    "EGE": "#FCD34D",                  # Bal Sarısı
    "İÇ ANADOLU": "#F59E0B",           # Amber
...
def prepare_data(df, gdf):
    ...
    merged["PF Pay %"] = (merged["PF Kutu"] / total_pf * 100).round(2) if total_pf > 0 else 0
    ...
    bolge_df["PF Pay %"] = (bolge_df["PF Kutu"] / total_pf * 100).round(2) if total_pf > 0 else 0
    ...
    return merged, bolge_df, total_pf, total_market
...
# FIGURE
# =============================================================================
def create_figure(gdf, manager, view_mode, total_pf):
    gdf = gdf.copy()
    ...
    for region in gdf["Bölge"].unique():
        ...
        fig.add_choropleth(
            ...
            showscale=False,
            customdata=list(zip(region_gdf["Şehir"], region_gdf["Bölge"], region_gdf["PF Kutu"])),
            hovertemplate="<b>%{customdata[0]}</b><br>Bölge: %{customdata[1]}<br>PF Kutu: %{customdata[2]:,.0f}<extra></extra>",
            name=region
        )
    ...
    if view_mode == "Bölge Görünümü":
        ...
        for region in gdf["Bölge"].unique():
            ...
            if total > 0:
                percent = (total / total_pf * 100) if total_pf > 0 else 0
        ...
        for idx, row in gdf.iterrows():
            if row["PF Kutu"] > 0:
                percent = (row["PF Kutu"] / total_pf * 100) if total_pf > 0 else 0
...
merged, bolge_df, total_pf, total_market = prepare_data(df, geo)
...
fig = create_figure(merged, selected_manager, view_mode, total_pf)
st.plotly_chart(fig, use_container_width=True)

# Genel İstatistikler
col1, col2, col3, col4 = st.columns(4)
with col1:
    st.metric("📦 PF Kutu", f"{total_pf:,.0f}")
with col2:
    st.metric("🏪 Toplam Kutu", f"{total_market:,.0f}")
with col3:
    pazar_payi = (total_pf / total_market * 100) if total_market > 0 else 0
    st.metric("📊 Genel Pazar Payı", f"%{pazar_payi:.1f}")
with col4:
